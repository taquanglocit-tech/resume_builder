  ...
  return (
    <nav className="h-14 w-full bg-resume-800 relative flex py-2.5 pl-2 md:pl-5 pr-1 nd:pr-4 items-center shadow-level-8dp z-20 print:hidden">
      ...
      <div className="flex-auto flex justify-between items-center xs:ml-3 md:ml-5">
        <NavBarMenu>
          <NavMenuItem
            caption={`Mẫu CV (${TOTAL_TEMPLATES_AVAILABLE})`}
            popoverChildren={<TemplateSelect />}
          />
          <NavMenuItem caption="Màu sắc" popoverChildren={<ThemeSelect />} />
          <StyledButton variant="text" onClick={handleSave}>
            Lưu
          </StyledButton>
        </NavBarMenu>
      ...
      >
        ...
        <MenuItem onClick={exportResumeData}>Xuất file</MenuItem>
        <MenuItem
          onClick={() => {
            if (fileInputRef.current) {
              const fileElement = fileInputRef.current as HTMLInputElement;
      ...
      <Toast
        ...
        content={'Dữ liệu Resume đã được nhập thành công.'}
      />
    </nav>
  );
};
